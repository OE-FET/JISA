...
public abstract class ResultTable implements Iterable<Row> {
    ...
    public static ResultTable loadFile(String path) throws IOException {
    ...
    public Column<?> getColumn(int index) {
        ...
        if (index < 0 || index >= columns.size()) {
            throw new IndexOutOfBoundsException(String.format("Column index %d is out of bounds. Must be 0 <= index <= %d.", index, columns.size() - 1));
        }
    ...
    public Object[][] asArray() {
        return stream().map(r -> columns.stream().map(c -> r.get(c)).toArray(Object[]::new)).toArray(Object[][]::new);
    }
