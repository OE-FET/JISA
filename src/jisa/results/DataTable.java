...
public abstract class DataTable implements Iterable<Row> {
    ...
    public static DataTable loadFile(String path) throws IOException {
        return DataList.loadFile(path);
    }
     ...
     */
    public <T> Map<T, DataList> split(RowEvaluable<T> splitBy) {

        Map<T, DataList> map = new LinkedHashMap<>();
     ...
     */
    public <T> Map<T, DataList> split(Column<T> splitBy) {
     ...
     */
    public List<DataList> directionalSplit(RowEvaluable<? extends Number> splitBy) {

        Integer        direction = null;
        Double         lastValue = null;
        List<DataList> list      = new LinkedList<>();
        DataList       table     = DataList.emptyCopyOf(this);
        list.add(table);
        ...
        for (Row row : this) {
            ...
            if (lastValue == null) {
                ...
                if (newDirection != direction && direction != 0) {
                    table = DataList.emptyCopyOf(this);
                    list.add(table);
                }
     ...
     * @return List of resulting tables, in alternating order of direction
     */
    public List<DataList> directionalSplit(Column<? extends Number> splitBy) {
     ...
     */
    public DataList filter(Predicate<Row> test) {
     ...
     * @return Sorted table
     */
    public DataList sorted(RowEvaluable<?> expression) {

        if (getRowCount() < 2) {
            return DataList.copyOf(this);
        }
     ...
     * @return Sorted table
     */
    public DataList sorted(Column<?> byColumn) {
     ...
     */
    public DataList transform(Map<Column<?>, RowEvaluable<?>> mappings) {

        DataList list = DataList.emptyCopyOf(this);
     ...
     */
    public DataList subTable(Column... columns) {
     ...
     * @return Copied sub-table
     */
    public DataList subTable(int start, int to) {
     ...
     * @return Copied sub-table
     */
    public DataList subTable(int start, int to, Column... columns) {
    ...
    public void outputBinary(String path, boolean compressed) throws IOException {
     ...
     */
    public Collector<Row, ?, DataList> collector() {
        return DataList.collect(this);
    }
