...
public class TwoErrorDataSet extends AbstractErrorDataSet<TwoErrorDataSet> implements DataSet2D, DataSetError, EditableDataSet {
    ...
    protected            DoubleArrayList xErrorsPos;
    protected            DoubleArrayList xErrorsNeg;
     ...
     */
    public TwoErrorDataSet(final DataSet another) {
     ...
     */
    public TwoErrorDataSet(final String name) {
        this(name, 0);
    }
     ...
     * @throws IllegalArgumentException if {@code name} is {@code null}
     */
    public TwoErrorDataSet(final String name, final int initalSize) {
        ...
        xErrorsPos = new DoubleArrayList(initalSize);
        xErrorsNeg = new DoubleArrayList(initalSize);
    }
     ...
     */
    public TwoErrorDataSet add(final double x, final double y) {
        return add(x, y, 0.0, 0.0, 0.0, 0.0, null);
    }
     ...
     * @return itself (fluent design)
     */
    public TwoErrorDataSet add(final double x, final double y, final double yErrorNeg, final double yErrorPos) {
        return add(x, y, yErrorNeg, yErrorPos, 0.0, 0.0, null);
    }
     ...
     * @return itself (fluent design)
     */
    public TwoErrorDataSet add(final double x, final double y, final double yErrorNeg, final double yErrorPos, final double xErrorNeg, final double xErrorPos) {
        return add(x, y, yErrorNeg, yErrorPos, xErrorNeg, xErrorPos, null);
    }
     ...
     * @return itself (fluent design)
     */
    public TwoErrorDataSet add(final double x, final double y, final double yErrorNeg, final double yErrorPos, final double xErrorNeg, final double xErrorPos, final String label) {
        lock().writeLockGuard(() -> {
            xValues.add(x);
            yValues.add(y);
            yErrorsNeg.add(yErrorNeg);
            yErrorsPos.add(yErrorPos);
            xErrorsNeg.add(xErrorNeg);
            xErrorsPos.add(xErrorPos);

            if (label != null && !label.isEmpty()) {
                addDataLabel(xValues.size() - 1, label);
            }

            getAxisDescription(DIM_X).add(x - xErrorNeg);
     ...
     * @return itself (fluent design)
     */
    public TwoErrorDataSet add(final double[] xValuesNew, final double[] yValuesNew, final double[] yErrorsNegNew, final double[] yErrorsPosNew, final double[] xErrorsNegNew, final double[] xErrorsPosNew) {
        ...
        lock().writeLockGuard(() -> {
            ...
            final int newElements = Math.min(Math.min(xErrorsPosNew.length, xErrorsNegNew.length), Math.min(Math.min(xValuesNew.length, yValuesNew.length), Math.min(yErrorsNegNew.length, yErrorsPosNew.length)));
            ...
            xErrorsNeg.setElements(addAt, xErrorsNegNew, 0, newElements);
            xErrorsPos.setElements(addAt, xErrorsPosNew, 0, newElements);
     ...
     */
    @Override
    public TwoErrorDataSet add(final int index, final double... newValue) {
    ...
    public TwoErrorDataSet add(int index, double x, double y) {
        return add(index, x, y, 0.0, 0.0, 0.0, 0.0, null);
    }
    ...
    public TwoErrorDataSet add(int index, double x, double y, double yErrorNeg, double yErrorPos) {
        return add(index, x, y, yErrorNeg, yErrorPos, 0.0, 0.0, null);
    }
     ...
     * @return itself (fluent design)
     */
    public TwoErrorDataSet add(int index, double x, double y, final double yErrorNeg, final double yErrorPos, final double xErrorNeg, final double xErrorPos) {
        return add(index, x, y, yErrorNeg, yErrorPos, xErrorNeg, xErrorPos, null);
    }
     ...
     * @return itself (fluent design)
     */
    public TwoErrorDataSet add(final int index, final double x, final double y, final double yErrorNeg, final double yErrorPos, final double xErrorNeg, final double xErrorPos, final String label) {
        lock().writeLockGuard(() -> {
            ...
            xErrorsNeg.add(indexAt, xErrorNeg);
            xErrorsPos.add(indexAt, xErrorPos);
     ...
     * @return itself (fluent design)
     */
    public TwoErrorDataSet add(final int index, final double[] x, final double[] y, final double[] yErrorNeg, final double[] yErrorPos) {
    ...
    public double getErrorNegative(final int dimIndex, final int index) {
        return dimIndex == DIM_X ? xErrorsNeg.elements()[index] : yErrorsNeg.elements()[index];
    }

    @Override
    public double getErrorPositive(final int dimIndex, final int index) {
        return dimIndex == DIM_X ? xErrorsPos.elements()[index] : yErrorsPos.elements()[index];
    }

    @Override
    public double[] getErrorsNegative(final int dimIndex) {
        return dimIndex == DIM_X ? xErrorsNeg.elements() : yErrorsNeg.elements();
    }

    @Override
    public double[] getErrorsPositive(final int dimIndex) {
        return dimIndex == DIM_X ? xErrorsPos.elements() : yErrorsPos.elements();
    }
     ...
     * @return itself (fluent design)
     */
    public TwoErrorDataSet increaseCapacity(final int amount) {
        lock().writeLockGuard(() -> {
     ...
     * @return itself (fluent design)
     */
    public TwoErrorDataSet remove(final int fromIndex, final int toIndex) {
        ...
        lock().writeLockGuard(() -> {
            ...
            xErrorsNeg.removeElements(fromIndex, clampedToIndex);
            xErrorsPos.removeElements(fromIndex, clampedToIndex);
     ...
     * @return itself (fluent design)
     */
    public TwoErrorDataSet resize(final int size) {
        lock().writeLockGuard(() -> {
            xValues.size(size);
            yValues.size(size);
            yErrorsPos.size(size);
            yErrorsNeg.size(size);
            xErrorsPos.size(size);
            xErrorsNeg.size(size);
        });
     ...
     */
    @Override
    public TwoErrorDataSet set(final DataSet other, final boolean copy) {
     ...
     * @return itself (fluent design)
     */
    public TwoErrorDataSet set(final double[] xValues, final double[] yValues, final double[] yErrorsNeg, final double[] yErrorsPos) {
     ...
     * @return itself (fluent design)
     */
    public TwoErrorDataSet set(final double[] xValues, final double[] yValues, final double[] yErrorsNeg, final double[] yErrorsPos, final boolean copy) {
     ...
     * @return itself (fluent design)
     */
    public TwoErrorDataSet set(final double[] xValues, final double[] yValues, final double[] yErrorsNeg, final double[] yErrorsPos, final int nSamples,
                               final boolean copy) {
     ...
     */
    @Override
    public TwoErrorDataSet set(final int index, final double... newValue) {
        if (newValue.length >= 4) {
     ...
     * @return itself (fluent design)
     */
    public TwoErrorDataSet set(final int index, final double x, final double y, final double yErrorNeg, final double yErrorPos) {
        lock().writeLockGuard(() -> {
