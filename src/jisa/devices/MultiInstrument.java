...
public interface MultiInstrument {
    ...
    default <I extends Instrument> I get(Class<I> type, int n) {
        ...
        return (I) getSubInstruments().stream().filter(i -> type.isAssignableFrom(i.getClass())).skip(n).findFirst()
                                      .orElseThrow(() -> new NoSuchElementException(
                                          String.format("No sub-instrument found of type %s with index %d", type.getSimpleName(), n)
                                      ));

    }

    /**
     * Returns the named sub-instrument that can be considered to be of the given class contained in this instrument.
     *
     * @param type Type of sub-instrument
     * @param name Name
     * @param <I>  Instrument class
     ...
     * @return Found instrument
     */
    default <I extends Instrument> I get(Class<I> type, String name) {

        return (I) getSubInstruments().stream()
                                      .filter(i -> type.isAssignableFrom(i.getClass()) && i.getName().equalsIgnoreCase(name))
                                      .findFirst()
                                      .orElseThrow(() -> new NoSuchElementException(
                                          String.format(
                                              "No sub-instrument found of type %s with name %s",
                                              type.getSimpleName(),
                                              name
                                          )));
     ...
     * @return Found instrument
     */
    default <I extends Instrument> I get(KClass<I> type, String name) {
        return get(JvmClassMappingKt.getJavaClass(type), name);
    }
