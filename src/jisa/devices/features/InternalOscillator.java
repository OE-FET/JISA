...
public interface InternalOscillator<F extends FSource> extends Feature, MultiInstrument {

    static void addParameters(InternalOscillator instrument, Class<?> target, ParameterList parameters) {

        FSource osc = instrument.getInternalReferenceOscillator();

        parameters.addValue("Internal Oscillator", "Use Internal Reference", instrument::isInternalReferenceEnabled, false, instrument::setInternalReferenceEnabled);
        parameters.addValue("Internal Oscillator", "Oscillator Frequency [Hz]", osc::getFrequency, 100.0, osc::setFrequency);
        parameters.addValue("Internal Oscillator", "Oscillator Amplitude [rms V]", osc::getAmplitude, 1.0, osc::setAmplitude);
